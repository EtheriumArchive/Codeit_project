...
FROM hackle_events
WHERE event_key NOT IN ('button', 'click_appbar_setting') -- 이벤트 목록 중 전처리대상, 세션 시작/종료 제외
GROUP BY event_key
ORDER BY count DESC
;
...
INNER JOIN hackle_events e ON p.session_id = e.session_id
WHERE user_id != ''
LIMIT 10
;

-- user_id별 session count
WITH hackle_join AS (
    SELECT
        user_id,
        p.session_id,
...
SELECT
    ...
    COUNT(event_key) AS count
FROM hackle_join
WHERE event_key = '$session_start' -- session_start가 session_end보다 더 많았기 때문!
GROUP BY user_id
ORDER BY count DESC
;

-- 시간차 집계 .. ㅎ.. with부터 안돌아가요...!
-- WITH hackle_join AS (
    SELECT
        user_id,
        p.session_id,
    ...
    FROM hackle_properties p
    INNER JOIN hackle_events e ON p.session_id = e.session_id
    WHERE user_id != '' OR event_key IN ('$session_start', '$session_end')
